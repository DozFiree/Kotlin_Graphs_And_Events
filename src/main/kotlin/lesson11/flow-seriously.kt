...
// 4. И также надо жёстко разделять события игрока Oleg от событий игрока Stas


// Для исправления данных проблем надо использовать flow оператор
...
fun main() = KoolApplication {
    ...
    addScene {
        ...
        server.events
            ...
            .onEach { event ->
                ...
                // as - приведение типа (мы уверены, что событие здесь будет AttackPressed т.к. выше - мы их отфильтровали)
        ...
        //Подписчик 4: TalckedToNpc и ChoiceSelected -> квест
        server.events
    ...
    addScene {
        ...
        if (server != null){
            ...
            // маршрутизация собитиый по активному игроку
            // activePlayeridFlow -> переходить flatMapLatest -> поток событий только для этого игрока
                ...
                Row{
                    Button("Сменить игрока") {
                        modifier.margin(end = 8.dp).onClick {
                            hud.activePlayerIdUi.value =
                                if (hud.activePlayerIdUi.value == "Oleg") "Oleg" else "Stas"
                        }
